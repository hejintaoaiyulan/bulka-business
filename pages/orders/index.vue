...
const handleStopPush = (order, evt) => {
  ...
  showModal("是否確定停止消息推送").then(() => {
    console.log(order);
    stopOrderPush({ order_no: order.order_no })
      .then(() => {
        Toast.success("操作成功");
        search();
      })
        ...
        <view class="order-item" v-for="order in dataList" :key="order.order_no">
          <view @click="handleToInfo(order)">
            ...
            <view class="order-content" style="margin: 20rpx 0">
              <view
                class="order-single-msg"
                v-if="order.goods.length === 1"
          ...
          <view class="order-operation">
            ...
            <view class="order-btns" v-if="![5, 6].includes(order.status)" @click="handleOperation">
              <uv-button
                size="small"
                v-if="order.push_status === 0"
                @click="handleStopPush(order, $event)"
                >停止推送</uv-button
              >
              <uv-button
                size="small"
                v-if="[1, 2, 3, 4].includes(order.status)"
                @click="handleCancel(order, $event)"
                >取消訂單</uv-button
              >
              <uv-button size="small" v-if="order.status === 2" @click="handleAccept(order, $event)"
                >接單</uv-button
              >
              <uv-button
                size="small"
                v-if="order.status === 3"
                @click="handleServingFood(order, $event)"
                >確認出餐</uv-button
              >
              <uv-button
                size="small"
                v-if="order.status === 4"
                @click="handleConnect(order, $event)"
                >聯繫客戶領取</uv-button
              >
            </view>
          </view>
